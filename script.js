...
function printResults(){
  ...
  // Build correct answers grid (always blue)
  let correctHTML = `
  ...
  for (let i=0; i<allQuestions.length; i++){
    ...
    correctHTML += `<div class="answer-chip" style="${baseStyle}${blueStyle}">${displayEq}</div>`;
  }
  correctHTML += `</div>`;
  ...
  const css = `
    <style>
      *{ box-sizing: border-box; }
      ...
      h1{ font-size: 24px; margin: 0 0 8px; page-break-before: always; }
  ...
  win.document.write(`
    <html>
      ...
      <body>
        ...
        <h1 style="margin-top:40px;">Correct Answers</h1>
        ${correctHTML}
